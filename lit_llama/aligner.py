...
This code is adapted from the lit_llama.adapter, which is an implementation of the paper:
LLaMA-Adapter: Efficient Fine-tuning of Language Models with Zero-init Attention
https://arxiv.org/abs/2303.16199

The main difference between the two is that the Aligner uses a global value embedding for the adapter, 
while the LLaMA-Adapter uses a series of layer wise learnable embeddings for the adapter.

The rest of the comment/explanation is also adapted from from lit_llama.adapter:

                                                                      cross-attention
    layer wise x                                                             |              Prefix (globally shared by all layers) 
                                                                             |
  ┌─────────────────┐                                                        |               ┌──────────────────┐
  ┆        x        ┆                                                        |               ┆   global prefix  ┆
  └─────────────────┘                                                        |               └──────────────────┘
    ...
    def __init__(self, config: LLaMAConfig, block_idx: int, global_value_embedding=None) -> None:
        ...
        if block_idx >= config.adapter_start_layer:
            ...
            if global_value_embedding is not None:
                ...
                # print("Aligner: using global value embedding for adapter at layer ", block_idx)
            else:
    ...
    def forward(
        ...
        if self.block_idx >= self.adapter_start_layer:
            if adapter_kv_cache is not None:
                ...
                # prefix = self.adapter_wte.weight[0,:].reshape(1, 1, self.n_embd) # for testing how many aligners to fit in one model
                aT = prefix.size(1)
